...
> **2026-09-21 状态：本基线已无法重跑。** 8/11 那份原始包已不在仓库（当前根目录只有 `mysql_inspection_v1_db01_192.168.100.80_3306_20260920_151817.tar.gz`，是另一次采集——同主机、挂载结构一致，但不能用来替换基线）。目录里的 `analysis.json` / `report_model.json` 因此停留在旧判定下，与当前代码已有 4 处已知不一致（`COMMON.SYSTEM.*_PRESSURE` 的峰值口径与置信度来源、文件系统容量改为只统计持久化挂载点）。逐条说明与新旧对照表见 `tests/baselines/mysql/current/README.md`；拿到 8/11 原始包后按该文件里的生成命令重跑并重算指纹。
